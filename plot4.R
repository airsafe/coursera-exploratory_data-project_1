...
# combine  first two columns (time and date) into a proper date format, 
# and the rest into numeric   
...
png(file = "plot4.png", width = 960, height = 960) # Open PNG device to place output in working directory
par(mfrow=c(2,2)) # Ensure 2x2 matrix of plots
# Above opened after graphics deviced opened to ensure proper output formatting
plot(sandbox48num$Date,sandbox48num$Global_active_power,"l", xlab = "", ylab = "Global Active Power") 
plot(sandbox48num$Date,sandbox48num$Voltage,"l", xlab = "datetime", ylab = "Voltage") 
...
plot(sandbox48num$Date,sandbox48num$Global_reactive_power,"l", xlab = "", ylab = "Global_reactive_power")
